window.onload = function () {
  ...
  const modifiedSpec = window.swaggerSpec ? JSON.parse(JSON.stringify(window.swaggerSpec)) : null;
  ...
  setTimeout(() => {
    ...
    if (container) {
      ...
      if (schemesSection && schemesServerContainer && authWrapper) {
        ...
        customUrlInput.placeholder = 'Enter custom server URL (e.g., https://api.example.com:8080)';
        ...
        const serversTitle = document.querySelector('.swagger-ui .servers-title');
        if (serversTitle) {
        ...
        const serverLabel = document.querySelector('.swagger-ui .servers label');
        if (serverLabel) {
  ...
  const observer = new MutationObserver(mutations => {
    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (node.nodeType === 1 && node.querySelector && node.querySelector('.modal-ux')) {
          // Modal opened, add API keys section
          setTimeout(() => {
            const modalContent = node.querySelector('.modal-ux-content');
            if (modalContent && !modalContent.querySelector('.modal-api-keys')) {
              ...
              fetch('/api/user-api-keys')
                ...
                .then(data => {
                  if (data.success) {
                    ...
                    title.style.cssText = 'color: #fff; margin: 0 0 12px 0; font-size: 14px;';
                    ...
                    if (apiKeys.length > 0) {
                      if (swaggerConfig.allow_full_key_retrieval) {
                        ...
                        apiKeys.forEach(key => {
                          ...
                          keyName.style.cssText = 'color: #fff; font-size: 12px;';

                          const keyInfo = document.createElement('div');
                          keyInfo.style.cssText = 'color: #adb5bd; font-size: 10px;';
                                ...
                                if (result.success && result.full_key) {
                                  ...
                                  if (authInput) {
                                    ...
                                    setTimeout(() => {
                                      ...
                                      authInput.focus();
                                      authInput.select();
                                      authInput.setSelectionRange(0, bearerToken.length);
                            ...
                            if (result.success && result.temp_key) {
                              ...
                              if (authInput) {
                                ...
                                setTimeout(() => {
                                  ...
                                  // Additional React-specific events
                                  authInput.dispatchEvent(
                                    new InputEvent('input', { bubbles: true, data: bearerToken })
                                  );
                    ...
                    if (apiKeys.length === 0 && !swaggerConfig.allow_temp_key_generation) {
